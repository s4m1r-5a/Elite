...
function Consultar(tipo, code) {
  ...
  if (tipo === "Nit") {
    datos.url =
      "https://api.misdatos.com.co/api/co/rues/consultarEmpresaPorNit";
    datos.dat = {
      nit: code,
    };
  } else {
...
$validationForm
  ...
  .on(
    "leaveStep",
    (e, anchorObject, currentStepNumber, nextStepNumber, stepDirection) => {
      ...
      $.ajax({
        ...
        success: function (data) {
          if (
            data != "Pin de registro invalido, comuniquese con su distribuidor!"
          ) {
            $(".h").attr("disabled", false);
            skdt = true;
          } else if ($("#ipin").val() != "") {
          ...
          data.dialogs.map((x, i) => {
            ...
            $("#contactos").append(
              `<div class="contact" id="${x.id.replace(/[^a-zA-Z 0-9]+/g, "")}">
                            ...
                            <input type="hidden" class="tiempo" value="${
                              x.last_time * 1000
                            }">
    ...
    success: function (data) {
      ...
      data.asesoresA &&
        data.asesoresA.map((x, i) => {
          ...
          $("#asesor-ventas" + y).html(
            `Separaciones ${x.ventas} - ${x.iniciales} Iniciales`
          );
...
if (window.location.pathname == `/links/pagos`) {
            ...
            success: function (data) {
              ...
              if (data.status) {
                ...
                $(".Total2").change(function () {
                  ...
                  var totalf = parseFloat(
                    $(".Totalf").html().replace(/\./g, "")
                  );
                ...
                $(".alert").show();
                $(".alert-message").html(
                  "<strong>Error!</strong> " + data.paquete
                );
                setTimeout(function () {
                  $(".alert").fadeOut(3000);
                }, 2000);
            ...
            $.ajax({
              ...
              success: function (data) {
                if (data) {
                  ...
                  SMSj(
                    "error",
                    `El bono fue rechazado, pongase en contacto con GRUPO ELITE`
                  );
                }
              },
            });
  ...
  var Pay = (forma) => {
    ...
    if (
      forma === "payu" &&
      (!$(".transaccion").html() || $(".transaccion").html() == 0)
    ) {
  ...
  $("#recbo").submit(function (e) {
    ...
    if (
      ...
      parseFloat($("#montorecibos").val()) < parseFloat($("#Total2").val())
    ) {
      SMSj(
        "error",
        "el monto de los recibos ingresados no corresponde al monto total a pagar"
      );
    } else {
...
if (window.location.pathname === `/links/comisiones` && rol.contador) {
  ...
  comisiones.on("click", "td:not(.control, .t)", function () {
    ...
    if (!aasesor && data.stado === 9) {
      ...
      SMSj(
        "error",
        "No puede seleccionar este pago ya que no pertenece al asesor"
      );
    } else {
      data.stado === 9
        ? fila.toggleClass("selected")
        : SMSj(
            "error",
            "No puede seleccionar este item ya que no se encuentra disponible"
          );
    }
  });
  var EstadoCC = (id, std, actualstd) => {
    if ((actualstd === 4 && rol.admin) || actualstd !== 4) {
      ...
      SMSj(
        "error",
        `Solicitud no pude ser procesada, no cuentas con los permisos`
      );
    }
  };
  var FaturaCobro = () => {
  ...
  var CuentaCobro = () => {
    ...
    if (BANCO) {
      ...
      SMSj(
        "error",
        "Debe seleccionar al menos una comicion, para generar una cuenta de cobro"
      );
      return false;
    } else {
      $("#idbank").val(ID);
...
if (window.location.pathname === `/links/reportes`) {
  ...
  var tableOrden = $("#datatable2").DataTable({
    ...
    drawCallback: function (settings) {
      ...
      filas.each(function (g, i) {
        ...
        resOrden =
          rol.admin &&
          g.tipobsevacion === "ANULADA" &&
          (g.estado == 9 || g.estado == 15);
  ...
  tableOrden.on("click", ".Std", function () {
    ...
    $("#Separacion").html(
      "Separado: " + moment(data.fecha).format("YYYY-MM-DD")
    );
    $("#Valor").html("Precio: $" + Moneda(data.valor));
    $("#PinKupon").html(data.pin ? `Cupon: ${data.pin}` : "Cupon: No aplica");
    $("#Porcentage").html(
      data.descuento ? `Descuento: ${data.descuento}%` : "Descuento: 0%"
    );
    $("#Descuento").html(
      data.ahorro ? `Ahorro: $${Moneda(data.ahorro)}` : "Ahorro: No aplica"
    );
    $("#TotalPrecio").html("Total: $" + Moneda(data.Total));
    $("#Mora").html(
      data.meses ? `Mes(es) mora: ${data.meses}` : "Mes(es) mora: Al dia"
    );
    $("#Deuda").html(
      data.deuda ? `Deuda: $${Moneda(data.deuda)}` : "Deuda: $0.000"
    );
    $("#Abono").html(
      data.abonos ? `Abonado: $${Moneda(data.abonos)}` : "Abonado: $0.000"
    );
                          ...
                          datos[i].ids
                            ...
                            <td colspan="15">
                                <nav class="nav nav-justified">
                                    ...
                                    <a class="nav-item nav-link py-0">RC-${
                                      datos[i].ids
                                    }</a>
    ...
    if (data.rcb && rol.subadmin) {
      ...
      $("#Modalimg .fotos").html(
        `<div class="foto" style="
                    ...
                    <tr class="op">
                        <th> 
                        <input type="hidden" name="img" class="imag" value="${
                          data.baucher
                        }">
  ...
  var Pago = (id, img, rcb) => {
    if (rcb === "null" && rol.subadmin) {
      var imagenes =
        img === null ? "" : img.indexOf(",") > 0 ? img.split(",") : img;
...
if (
  window.location.pathname ==
    `/links/editordn/${window.location.pathname.split("/")[3]}` &&
  !rol.externo
) {
  minDateFilter = "";
  maxDateFilter = "";
          ...
          success: function (data) {
            if (data) {
              if (data.valor >= valor) {
                ...
                SMSj(
                  "error",
                  "No es posible cambiar el producto por uno de menor valor"
                );
  ...
  $.ajax({
    ...
    data.asesores.map((x, v) => {
      ...
      asesores.append(
        new Option(`${x.fullname}  CC ${x.document}`, x.id, pd, pd)
      );
    });
    ...
    data.clientes.map((x, v) => {
      ...
      clientes.append(
        new Option(`${x.nombre}  CC ${x.documento}`, x.idc, pd, pd)
      );
    });
        ...
        .each(function (group, i) {
          if (last !== group) {
            $(rows)
              .eq(i)
              .before(
                '<tr class="group"><td colspan="8">' + $(group).val() ||
                  group + "</td></tr>"
              );
  ...
  crearcartera.on("click", "tr a", function () {
    ...
    if (!nf && !ni) {
      ...
      SMSj(
        "info",
        "No es posible eliminar toda la finaciacion sin antes eliminar toda la inicial"
      );
      return;
    }
  ...
  crearcartera.on("click", ".guardar", function () {
    $("#crearcartera")
      .find(
        "#ahorro, #inicialcuotas, #financiacion, #ini, #fnc, #total, #inicial"
      )
      .prop("disabled", false);
  ...
  crearcartera.on("change", ".tabl .cuota", function () {
    ...
    if (tipo === "SEPARACION") {
      if (TotalSprc === TotalIni + TotalFnc) {
        ...
        TotalSprc = parseFloat(
          $("#Separar").val("1.000.000").replace(/\./g, "")
        );
        if (nu == 0) {
        ...
        .filter((c, i) => {
          var e =
            $(i).find(`.cuota`).val() === undefined
              ...
              : $(i).find(`.cuota`).val();
          return (
            $(i).find(`.tipo`).val() === "INICIAL" &&
            parseFloat(e) !== realcuotai &&
            realcuotai > 0
          );
        })
        .map((c, i) => {
  ...
  var CONT = () => {
    ...
    var ini =
      Math.sign(totalCuotaInicial - Separacion) > 0
        ? totalCuotaInicial - Separacion
        : 0;
...
if (
  ...
  $(document).ready(function () {
    ...
    var total =
      parseFloat($(".totalp").html()) - parseFloat($(".ahorro").html());
...
if (window.location.pathname == `/links/cartera`) {
    ...
    $("#financiacion-btn").click(function () {
      ...
      if ($("#Separar").val()) {
        FINANCIAR(
          "FINANCIACION",
          c,
          parseFloat($("#Separar").val().replace(/\./g, ""))
        );
      } else {
  ...
  if (rol.auxicontbl || !rol.externo) {
    ...
    $.ajax({
      ...
      data.asesores.map((x, v) => {
        asesores.append(
          new Option(`${x.fullname} CC ${x.document} `, x.id, false, false)
        );
      });
      //clientes.append(new Option(`Selecciona un Cliente`, 0, true, true))
      data.clientes.map((x, v) => {
        clientes.append(
          new Option(`${x.nombre} CC ${x.documento} `, x.idc, false, false)
        );
      });
    });
    $("#min, #max").on("keyup", function () {
    ...
    crearcartera.on("change", ".tabl .cuota", function () {
      ...
      $("#crearcartera .tabl tr").each((e, i) => {
        ...
        var cuota =
          tpo !== undefined
            ? parseFloat($(i).find(`.cuota`).val().replace(/\./g, ""))
            : 0;
...
if (
          ...
          success: function (data) {
            $("#datatable2")
              ...
              .ajax.reload(function (json) {
                ...
                SMSj(
                  "success",
                  "Nuevo precio del metro cuadrador establecido correctamente"
                );
  ...
  var tabledit = $("#datatabledit").DataTable({
    ...
    columns: [
      ...
      },
      {
        data: "valor",
        render: function (data, method, row) {
          return `<span class="badge badge-pill badge-dark valor2">${Moneda(
            data
          )}</span>
      ...
      },
      {
        data: "inicial",
        render: function (data, method, row) {
          return `<span class="badge badge-pill badge-dark inicial2">${Moneda(
            data
          )}</span>
  ...
  var Recorrer = (n) => {
    ...
    $("#datatabledit .totalproyect").val(
      "$" + Moneda(Math.round(valorproyecto))
    );
  ...
  tabledit.on(
    "change",
    "tr input:not(.edi, .c), textarea, select:not(.edi, .c)",
    function () {
      var fila = $(this).parents("tr");
      var data = tabledit.row(fila).data();
      if (data.estado == 9 || data.estado == 15) {
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data) {
          ...
          $(
            `#datatabledit select[name="categoria"] option[value='${data.categoria}']`
          ).prop("selected", true);
          $(
            `#datatabledit .comision option[value='${data.comision.toString()}']`
          ).prop("selected", true);
          $(
            `#datatabledit .maxcomis option[value='${data.maxcomis.toString()}']`
          ).prop("selected", true);
          $(
            `#datatabledit .linea1 option[value='${data.linea1.toString()}']`
          ).prop("selected", true);
          $(
            `#datatabledit .linea2 option[value='${data.linea2.toString()}']`
          ).prop("selected", true);
          $(
            `#datatabledit .linea3 option[value='${data.linea3.toString()}']`
          ).prop("selected", true);
          $(`#xcntag option[value='${data.porcentage.toString()}']`).prop(
            "selected",
            true
          );
          ...
          $(".mzs").val(data.mzs);
          $(`#datatabledit input[name="separacion"]`).val(
            Moneda(data.separaciones)
          );
          $(`#datatabledit input[name="incentivo"]`).val(
            Moneda(data.incentivo) || 0
          );
          ...
          $("#rangofechas span").html(
            fi.format("ll") + " - " + ff.format("ll")
          );
...
if (window.location.pathname == `/links/orden` && !rol.externo) {
  ...
  var Meses = (extra) => {
    ...
    var months =
      fechs.getMonth() -
      fch.getMonth() +
      12 * (fechs.getFullYear() - fch.getFullYear());
    months += extra;
  ...
  $(document).ready(function () {
          ...
          success: function (data) {
            if (data.length > 0) {
              ...
              data[0].movil.indexOf(" ") > 0
                ? $(`#${card} .pais option[value='${pais[0]}']`).prop(
                    "selected",
                    true
                  )
                ...
                $.ajax({
                  ...
                  success: function (data) {
                    if (data) {
                      ...
                      $(`#${card} .nombres`).val(
                        $("#AddCliente .nombr").val().toLocaleUpperCase()
                      );
                      $(`#${card} .movil`)
    ...
    var Años = (inic, dia, month) => {
      ...
      jun = 0;
      var t1 = new Date(
        moment(fch).add(inic, "month").startOf("month")
      ).valueOf();
      var t2 = new Date(
        moment(fch).add(month, "month").endOf("month")
      ).valueOf();
      var r = $("#Emeses").val() || 0;
      var data = new Array();
      var q = 0;
    ...
    $(".edi").on("change", function () {
      ...
      if ($(this).attr("id") === "abono") {
        if (parseFloat($(this).cleanVal()) < cpara || !$("#abono").val()) {
          ...
          parseFloat($(this).cleanVal()) > precio ||
          !$("#abono").val()
        ) {
          $("#abono").val(Moneda(cpara));
        ...
        { data: "n" },
        {
          ...
          render: function (data, method, row) {
            return (
              ...
              `<input value="${moment(data).format(
                "YYYY-MM-DD"
...
if (window.location == `${window.location.origin}/links/solicitudes`) {
  ...
  table.on("click", "td:not(.t)", function () {
    ...
    if (Array.isArray(imagenes)) {
      ...
      $("#descargaimg .imag").remove();
      $("#descargaimg").html(
        `<a class="imag" href="${data.img}" target="_blank"><span class="badge badge-dark">Img</span></a>`
      );
      $("#Modalimg .fotos").append(
        `<div class="foto" style="
                    width: 100%;
    ...
    $("#Modalimg .fecha").html(
      moment.utc(data.fech).format("YYYY-MM-DD HH:mm")
    );
    ...
    $("#aprobadoEl").html(
      data.aprobado && "Aprobado el " + moment(data.aprobado).format("llll")
    );

    let esFecha = moment(data.fecharcb, true).isValid();
    !esFecha &&
      alert(
        'Debe establecer una fecha al recibo a continuacion donde se muestra "Fecha inválida"'
      );

    var nuevaFechaRecibo = moment(
    ...
    var diferenciaDeFechas = Math.abs(
      moment(data.fecharcb).diff(moment(data.date), "days")
    );
    ...
    switch (data.stado) {
      ...
      case 6:
        $("#Modalimg .estado").html(
          `<span class="badge badge-pill badge-danger">Declinada</span>`
        );
        break;
      case 3:
        $("#Modalimg .estado").html(
    ...
    $(".dropdown-item").on("click", function () {
      ...
      if (accion === "Declinar" && rol.contador) {
        ...
        if (!idExtracto) {
          alert(
            "Debe seleccionar un extrato antes de intentar asociar al pago"
          );
          return false;
        }
        ...
        if (!rol.externo && !nuevaFechaRecibo) {
          alert(
            "Debe establecer una fecha al recibo antes de realizar esta accion"
          );
          return false;
        }
                ...
                doc.autoTable({
                  ...
                  body: [
                    ...
                    },
                    {
                      ...
                      city: "CUOTA #",
                      expenses:
                        data.ncuota === null ? "NO APLICA" : data.ncuota,
                    },
                    {
                      id: "F PAGO",
                      name: data.formap,
                    ...
                    },
                    {
                      ...
                      email:
                        data.producto === null ? "R5 0" : "R5 " + data.producto,
                      city: "MONTO",
                      expenses: "$" + Moneda(bon),
                    },
                    {
          ...
          $(rows)
            ...
            .before(
              `<tr class="group" style="background: #7f8c8d; color: #FFFFCC;">
                ...
                group.nam
              }', '${group.clu}')"><i class="fas fa-trash-alt"></i> Declinar</a>
  ...
  var BuscarFechaRcb = (start, extra) => {
    ...
    let buscar = new RegExp(
      `${fechaAnt}|${fechaRCB}|${fechaPos1}|${fechaPos2}|${fechaPos3}|${fechaPos4}`
    );
    //console.log(buscar, extra, start)
    extra
      ? BancoExt.column(0).search(extra).draw()
...
if (window.location == `${window.location.origin}/links/red` && !rol.externo) {
  ...
  var red2 = $("#red2").DataTable({
    ...
    columns: [
      ...
      { data: "cli" },
      {
        data: "sucursal",
        render: function (data, method, row) {
          return `<input type="text" class="text-center edir" id="${
            row.pin
          }" style="width:40px"
  ...
  red.on("click", "tr", function () {
    ...
    red
      ...
      .each(function (group, i) {
        if (
          last == group.toLocaleLowerCase().trim() &&
          $(rows).eq(i).is(":hidden")
        ) {
          $(rows).eq(i).show();
...
/////////////////////////////* CLIENTES */////////////////////////////////////////////////////////////////////
if (
  window.location == `${window.location.origin}/links/clientes` &&
  !rol.externo
) {
  $(".sidebar-item").removeClass("active");
  $(`a[href='${window.location.pathname}']`).parent().addClass("active");
  ...
  var clientes = $("#clientes").DataTable({
    ...
    initComplete: function (settings, json, row) {
      $("#datatable_filter").prepend(
        "<h3 class='text-center mt-2'>CLIENTES</h3>"
      );
    },
    columns: [
      {
      ...
      $.ajax({
        ...
        success: function (data) {
          if (data) {
            ...
            SMSj(
              "error",
              "No es posible eliminar este cliente ya que cuenta conuna separacion"
            );
          }
        },
      });
...
/////////////////////////////* CUPONES */////////////////////////////////////////////////////////////////////
if (
  window.location == `${window.location.origin}/links/cupones` &&
  !rol.externo
) {
  $(".sidebar-item").removeClass("active");
  $(`a[href='${window.location.pathname}']`).parent().addClass("active");
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if (data) {
          SMSj(
            "success",
            `Cupon de descuento enviado al solicitante correctamente`
          );
        } else {
          SMSj(
            "error",
          ...
          columns: [
            ...
            },
            {
              ...
              render: function (data, method, row) {
                return (
                  "$" +
                  Moneda(Math.round((data - row.inicial) / (maxcuotas - 1)))
                );
              },
            },
          ],
