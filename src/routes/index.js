...
_Siempre que lo desees puedes volver al *menú principal*. 🔙 Enviándome *"#"*_`;
    ...
    if (body == 0) {
      ...
      if (recibos.length) {
        ...
        }*, en el sistema nos registran *${
          recibos.length
        }* recibos🧾 de caja los cuales se resumen a continuación_ \n\n`;
        recibos.map((e, i) => {
          body += `_${e.stado !== 4 ? "~" : ""}*RC${
            e.ids
          }* por un valor de💵 *$${Moneda(e.monto)}*${
            e.stado !== 4 ? "~" : ""
          }_\n`;
        });
      ...
      const cel = chatId.replace("@c.us", "").slice(-10);
      const recibos =
        await pool.query(`SELECT s.monto, c.nombre, l.valor, p.ahorro
      ...
      if (recibos.length) {
        ...
        let body = `_😁Hola *${
          recibos[0].nombre.split(" ")[0]
        }*, Su salso a la fecha es de *$${Moneda(
          recibos[0].valor - recibos[0].ahorro - saldo
        )}*_
                \n_*${NumeroALetras(
                  recibos[0].valor - recibos[0].ahorro - saldo
                )}.*_`;
      ...
      const Code = await pool.query(
        `SELECT * FROM clientes WHERE code = ?`,
        body.trim()
      );
      if (Code.length) {
...
router.post("/confir", async (req, res) => {
  ...
  const pin = await pool.query(
    "SELECT * FROM payu WHERE reference_sale = ?",
    reference_sale
  );
  var d = "";
  if (pin.length > 0) {
    console.log("si existe");
    await pool.query("UPDATE payu set ? WHERE reference_sale = ?", [
      r,
      reference_sale,
    ]);
  } else {
  ...
  if (state_pol == 4 && d) {
    ...
    EnviarWTSAP(
      phone,
      `Solicitud de pago ${reference_sale} rechazada, ${response_message_pol}`
    );
  }
});
...
router.post(`/venta`, async (req, res) => {
  ...
  const cliente = await pool.query(
    "SELECT * FROM clientes WHERE email = ? AND movil = ?",
    [buyerEmail, telephone]
  );
  if (cliente.length > 0) {
...
async function PagosAbonos(Tid, user) {
  //u.
  const SS =
    await pool.query(`SELECT s.fech, c.fechs, s.monto, u.pin, c.cuota, u.nrango, c.mora, 
  ...
  if (S.concepto === "ABONO") {
    ...
    if (Cuotas.length > 0 && monto > 0) {
      ...
      var sql = `UPDATE cuotas SET estado = 13, mora = 0, fechapago = '${moment(
        fech2
      ...
      Cuotas.map((c) => {
        ...
        if (montocuotas >= cuot) {
          ...
          var cuo =
            montocuotas > c.mora ? c.cuota - (montocuotas + c.mora) : c.cuota;
      ...
      if (Cuotas.length > 0 && montocuotas > 0) {
        ...
        Cuotas.map((c) => {
          ...
          if (montocuotas >= cuot) {
            ...
            sql2 =
              `UPDATE cuotas SET estado = 3, fechapago = '${moment(
                fech2
              ).format("YYYY-MM-DD HH:mm")}', cuota = ` +
              cuo +
              ", mora = " +
              mor +
              ", abono = " +
